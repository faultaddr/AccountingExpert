...
class FullMonthView(context: Context) : MonthView(context) {
    ...
    override fun onDrawText(
        ...
        val top = y - mItemHeight / 3
        val isInRange = isInRange(calendar)
        calendar.detailInfo = "100.32"
        if (isSelected) {
            canvas.drawText(
            ...
            canvas.drawText(
                calendar.detailInfo, cx.toFloat(),
                mTextBaseLine + y + mItemHeight / 5, mSelectedDetailPaint
            )
        } else if (hasScheme) {
            canvas.drawText(
                calendar.day.toString(), cx.toFloat(), mTextBaseLine + top,
            ...
            canvas.drawText(
                calendar.detailInfo, cx.toFloat(),
                mTextBaseLine + y + mItemHeight / 5, mCurMonthDetailTextPaint
            )
        } else {
            canvas.drawText(
                calendar.day.toString(), cx.toFloat(), mTextBaseLine + top,
            ...
            if (calendar.isCurrentMonth) {
                canvas.drawText(
                    calendar.detailInfo, cx.toFloat(),
                    mTextBaseLine + y + mItemHeight / 5,
                    if (calendar.isCurrentDay && isInRange) mCurDayDetailInfoTextPaint else mCurMonthDetailTextPaint
                )
            }
        }
    }
